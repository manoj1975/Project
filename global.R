...
# Following functions to check. The return of column will be like this: [nextword] [ngrams.scores]. 
...
scoreNgrams <- function(x, nrows=5) {
  ...
  if (nrow(df) > 0) {
    ...
    # Calculate Stupid Back Off Rank result.
    alpha <- 0.4
    df$score <- round(
    ...
    # If result is less than 5, append highest frequenct single word to it.
    if (nrow(df)<5) {
      rest <- 5-nrow(df)
      df <- merge(df, ngram1[1:rest,], by.x=c("nextword","rank"), by.y=c("w1","freq"), all=TRUE, sort = FALSE)
    }
  } else {
    df <- ngram1[1:5,]
    colnames(df) <- c("nextword", "rank")
  }
  return(df)
}
